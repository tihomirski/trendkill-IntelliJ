/*
 * $Header: //depot/FXCM/New_CurrentSystem/Main/FXCM_SRC/TRADING_SDK/tradestation/src/main/fxts/stations/trader/ui/actions/CreateMarketOrderAction.java#2 $
 *
...
import fxts.stations.trader.ui.dialogs.CreateMarketOrderDialog;
 ...
 */
public class CreateMarketOrderAction extends ATradeAction implements ISignalListener {
    private static final String ACTION_NAME = "CreateMarketOrderAction";
    /**
     * Singeleton of this class
     */
    private static final CreateMarketOrderAction CREATE_MARKET_ORDER_ACTION = new CreateMarketOrderAction();
    /**
     ...
     * Dialog
     */
    private CreateMarketOrderDialog mDlg;
     ...
     */
    private CreateMarketOrderAction() {
        super(ACTION_NAME);
        try {
    ...
    public void actionPerformed(ActionEvent aEvent) {
        ...
        if (rate != null && rate.isTradable()) {
            ...
            checkActionEnabled();
            if (mEnabled && mCanAct) {
                mDlg = new CreateMarketOrderDialog(TradeApp.getInst().getMainFrame());
                if (mAccount == null) {
                    try {
                        Accounts accounts = TradeDesk.getInst().getAccounts();
                        if (!accounts.isEmpty()) {
                            mAccount = ((Account) accounts.get(0)).getAccount();
                        }
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                }
                mDlg.setAccount(mAccount);
                mDlg.setCurrency(TradeApp.getInst().getRatesFrame().getSelectedCurrency());
                String command = aEvent.getActionCommand();
                if (Side.SELL.getName().equals(command)) {
                ...
                if (res == JOptionPane.OK_OPTION) {
                    ...
                    IRequest request;
                    if (mDlg.isAtBest()) {
                        request = requestFactory.createTrueMarketOrder(mDlg.getCurrency(),
                                                                       mDlg.getAccount(),
                                                                       mDlg.getSide(),
                                                                       mDlg.getAmount(),
                                                                       mDlg.getCustomText());
                    } else {
                        request = requestFactory.createMarketOrder(mDlg.getCurrency(),
                                                                   mDlg.getAccount(),
                                                                   mDlg.getSide(),
                                                                   mDlg.getAmount(),
                                                                   mDlg.getCustomText(),
    ...
    public static Action newAction(String aCommandString) {
        Action action = CREATE_MARKET_ORDER_ACTION.new MarketOrderAction();
        if (aCommandString != null) {
            action.putValue(ACTION_COMMAND_KEY, aCommandString);
        }
        CREATE_MARKET_ORDER_ACTION.mInnerActions.add(action);
        CREATE_MARKET_ORDER_ACTION.checkActionEnabled();
        return action;
    }
    ...
    private void setAccountByIndex(int aIndex) {
        try {
            ...
            if (aIndex < 0) {
                ...
                if (accounts != null && accounts.size() > aIndex) {
     ...
     * interface controls
     */
    private class MarketOrderAction extends AbstractAction {
        /**
         * Invoked when an action occurs
         */
        public void actionPerformed(ActionEvent aEvent) {
            CreateMarketOrderAction.this.actionPerformed(aEvent);
        }
    }
    ...
    private class TableListener implements ITableListener {
        ...
        public void onRemoveTable(ITable aTable) {
            try {
                String tableName = aTable.getName();
                if (AccountsFrame.NAME.equals(tableName)) {
                    mAccountsCurRow = -1;
