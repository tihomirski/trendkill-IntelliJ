/*
 * $Header: //depot/FXCM/New_CurrentSystem/Main/FXCM_SRC/TRADING_SDK/fxcm-api/src/main/QATest.java#23 $
 *
...
public class QATest {
    ...
    private Boolean runTest(boolean aStatus) {
        //testPurgeAll();
            ...
            public void process(MarketDataSnapshot aMarketDataSnapshot) {
                try {
                    if (mOrder && !mAccounts.isEmpty()) {
                        ...
                        calendar.add(Calendar.DATE, 30); //roll fwd 30 days
                        os2.setExpireDate(new UTCDate(calendar.getTime()));
                        mRequestId = mFxcmGateway.sendMessage(os2);
                        System.out.println("client: entry order requestId = " + mRequestId);
                    }
                } catch (Exception e) {
            ...
            public void process(MarketDataSnapshot aMarketDataSnapshot) {
                try {
                    ...
                    if (mOrder && !mAccounts.isEmpty() && symbol.indexOf("EUR/USD") == 0) {
                        ...
                        double market = aMarketDataSnapshot.getAskClose();
                        double stop = add(market, .00002, aMarketDataSnapshot.getInstrument().getSymbol());
                        double limit = add(market, .00004, aMarketDataSnapshot.getInstrument().getSymbol());
